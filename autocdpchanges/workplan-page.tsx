...
interface WorkplanPageProps {
  ...
  /** When everything is done, we render the celebration empty state. */
  allComplete?: boolean;
}
      ...
      <PageHeader
        ...
        actions={
          <select
            value={view}
            onChange={(e) =>
              setView(e.target.value as (typeof VIEWS)[number])
            }
      ...
      <div className="flex items-start justify-between gap-4">
        ...
        <div className="flex items-center gap-2 text-xs">
          <CountChip
            ...
            value={group.counts.calls}
            tone="emerald"
          />
          <CountChip
            icon={<MessageCircle className="h-3 w-3" />}
            value={group.counts.texts}
            tone="sky"
          />
          <CountChip
            icon={<Mail className="h-3 w-3" />}
            value={group.counts.emails}
            tone="indigo"
          />
          <CountChip
            icon={<Video className="h-3 w-3" />}
            value={group.counts.videos}
            tone="rose"
          />
          <CountChip
            icon={<CheckCircle2 className="h-3 w-3" />}
            value={group.counts.tasks}
            tone="amber"
          />
        </div>
      </div>
