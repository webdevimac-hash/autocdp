...
export interface FunnelStage {
  ...
  label: string;
  count: number;
  /** Number of leads in this stage with overdue follow-ups. */
  warnings?: number;
  /** Number of "hot" leads in this stage (fire icon). */
  hot?: number;
  /** Delivered uses "people reached" instead of fire — count of customers. */
  customers?: number;
}
...
interface SalesEngagementHubProps {
  stages: FunnelStage[];
  /** Drop-off counts between stages. Length === stages.length − 1. */
  dropoffs: number[];
  channels: ChannelTotal[];
  /** Overall engagement count shown in the small badge at the far left. */
...
/**
 * The chevron arrow between funnel stages. We draw it with SVG so the value
 * sits cleanly underneath without overlapping the chevron.
 */
