/**
 * POST   /api/integrations/general-crm/sync — Trigger sync.
 * DELETE /api/integrations/general-crm/sync — Disconnect.
 */
...
export async function POST(req: NextRequest) {
  ...
  const { data: conn } = await svc
    ...
    .eq("provider", "general_crm")
    .eq("status", "active")
    .maybeSingle();

  if (!conn) return NextResponse.json({ error: "General CRM not connected" }, { status: 400 });
